...
apt_info = {
    "성수롯데캐슬파크":{"seq":51206,"key":'75636954131525551206'},
    "성수아이파크":{"seq":51203,"key":'75336954131536651203'},
    "금호2차":{"seq":279,"key":'759369543199554279'},
    "금강아미움":{"seq":53513,"key":'75336954122989653513'},
    "서울숲힐스테이트":{"seq":20117205,"key":'75536879887329220117205'}
}
...
st.title(f"성수역 유동인구 & {select_apt}시세")
...
# 유동인구 (천 명 단위)
ax2 = ax1.twinx()
ax2.set_ylabel("성수역 유동인구 (천명)", color="red")
ax2.plot(merged["연도"], merged["유동인구(천명)"], color="red", linestyle="--", marker="s")
ax2.tick_params(axis='y', labelcolor="red")

plt.title(f"성수역 유동인구 {select_apt} 시세")
plt.grid(True)
st.pyplot(fig)
...
yeouido_df = data[data["지하철역"] == "성수"].copy()
...
# 결과
st.title("연-월별 성수역 아파트 시세 & 유동인구 Spearman 상관분석")
...
st.markdown("## 분석 요약: 성수역 유동인구 vs 아파트 시세 (Spearman 상관분석)")
...
- 즉, 성수역 유동인구가 많을수록 아파트 시세가 높아지는 경향이 관찰되었습니다
