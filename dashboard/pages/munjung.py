...
apt_info = {
    "올림픽훼미리":{"seq":2287,"key":'7573695431252582287'},
    "가락한신":{"seq":2267,"key":'7573695431259982267'},
    "문정한전현대대":{"seq":2284,"key":'7543695431253692284'},
    "문정푸르지오1차":{"seq":2278,"key":'7583695431255912278'},
    "건영":{"seq":2276,"key":'7563695431256652276'}
}
...
st.title(f"문정역 유동인구 & {select_apt}시세")
...
# 유동인구 (천 명 단위)
ax2 = ax1.twinx()
ax2.set_ylabel("문정역 유동인구 (천명)", color="red")
ax2.plot(merged["연도"], merged["유동인구(천명)"], color="red", linestyle="--", marker="s")
ax2.tick_params(axis='y', labelcolor="red")

plt.title(f"문정역 유동인구 {select_apt} 시세")
plt.grid(True)
st.pyplot(fig)
...
# 문정역 데이터 (연도+월별 집계 + 2025 보정)
magok_df = data[data["지하철역"] == "문정"].copy()
...
# 결과
st.title("연-월별 문정역 아파트 시세 & 유동인구 Spearman 상관분석")
...
ax.set_title("문정역 유동인구(천명) vs 아파트 시세(억단위)")
st.pyplot(fig)




st.markdown("## 분석 요약: 문정역 유동인구 vs 아파트 시세 (Spearman 상관분석)")
...
- 즉, 문정역 유동인구가 많을수록 아파트 시세가 높아지는 경향이 관찰되었습니다
