...
apt_info = {
    "신동아":{"seq":1484,"key":'7543695431549691484'},
    "삼익":{"seq":1482,"key":'7523695431550431482'},
    "한아름":{"seq":1483,"key":'7533695431550061483'},
    "동익":{"seq":1480,"key":'7503695431551171480'},
    "강남더샵포레스트":{"seq":20340231,"key":'75136879062133020340231'}
}
...
st.title(f"수서역 유동인구 & {select_apt}시세")
...
# 유동인구 (천 명 단위)
ax2 = ax1.twinx()
ax2.set_ylabel("수서역 유동인구 (천명)", color="red")
ax2.plot(merged["연도"], merged["유동인구(천명)"], color="red", linestyle="--", marker="s")
ax2.tick_params(axis='y', labelcolor="red")

plt.title(f"수서역 유동인구 {select_apt} 시세")
plt.grid(True)
st.pyplot(fig)
...
# 성수역 데이터 (연도+월별 집계 + 2025 보정)
yeouido_df = data[data["지하철역"] == "수서"].copy()
...
# 결과
st.title("연-월별 수서역 아파트 시세 & 유동인구 Spearman 상관분석")
...
ax.set_title("수서역 유동인구(천명) vs 아파트 시세(억단위)")
st.pyplot(fig)




st.markdown("## 분석 요약: 수서역 유동인구 vs 아파트 시세 (Spearman 상관분석)")
...
- 즉, 수서역 유동인구가 많을수록 아파트 시세가 높아지는 경향이 관찰되었습니다
