...
apt_info = {
    "광장아파트":{"seq":955,"key":'755369543174542955'},
    "미성아파트":{"seq":959,"key":'759369543174394959'},
    "롯데캐슬엠파이어":{"seq":617938,"key":'758369520346171617938'},
    "수정아파트":{"seq":964,"key":'754369543174209964'},
    "여의도자이":{"seq":20090966,"key":'75636879984413520090966'}
}
...
st.title(f"여의도역 유동인구 & {select_apt}시세")
...
# 여의도역만 필터링
yeouido_df = data[data["지하철역"] == "여의도"]
pop_df = yeouido_df.groupby("연도")["유동인구"].sum().reset_index()
...
# 유동인구 (천 명 단위)
ax2 = ax1.twinx()
ax2.set_ylabel("여의도역 유동인구 (천명)", color="red")
ax2.plot(merged["연도"], merged["유동인구(천명)"], color="red", linestyle="--", marker="s")
ax2.tick_params(axis='y', labelcolor="red")

plt.title(f"여의도역 유동인구 {select_apt} 시세")
plt.grid(True)
st.pyplot(fig)
...
fig, ax = plt.subplots()
stats.probplot(sample, dist="norm", plot=ax) # 정규분포를 기준으로 qqplot 그리는 함수
ax.set_title(f"{selected_month}월 Q-Q Plot")
st.pyplot(fig)

## Shapiro-Wilk 정규성 검정 실행
# 데이터가 정규분포를 따르는지
# stat = 검정통계량 W (1에 가까울수록 정규분포)
# p-value > 0.05 --> 정규분포를 따름
# else --> 따르지 않음
# p-value = 정규분포라 가정했을 때 일탄한 데이터가 나올 확률
...
# 여의도역 데이터 (연도+월별 집계 + 2025 보정)
yeouido_df = data[data["지하철역"] == "여의도"].copy() # 원본 데이터 변경 방지
yeouido_df["월"] = data["사용월"].astype(str).str[4:].astype(int) # 연도와 월을 분리
yeouido_df["연도"] = data["사용월"].astype(str).str[:4].astype(int)

# 연도-월 단위 집계
pop_ym = yeouido_df.groupby(["연도", "월"])["유동인구"].sum().reset_index()
...
# 병합
merged_df = pd.merge(price_ym, pop_ym[["연도", "월", "유동인구(천명)"]], on=["연도", "월"], how="inner") # 공통된 연도+월이 있는 행만 남기기 위해

# Spearman 상관관계 분석(데이터가 정규분포하지 않기 때문에)
rho, p = stats.spearmanr(merged_df["가격(억단위)"], merged_df["유동인구(천명)"])
#rho: 상관계수 (–1 ~ +1 범위)-->  0에 가까우면 관계 없음 양수면 함께 증가 음수면 반대로 움직임
# p: 상관계수가 우연히 발생할 확률 작을수록 통계적으로 유의미

# 결과
st.title("연-월별 여의도 아파트 시세 vs 유동인구 Spearman 상관분석")
...
ax.set_title("여의도역 유동인구(천명) vs 아파트 시세(억단위)")
st.pyplot(fig)




st.markdown("## 분석 요약: 여의도역 유동인구 & 아파트 시세 (Spearman 상관분석)")
...
- 상관계수  = {rho:.4f}는 중간 정도의 양의 상관관계를 의미합니다
- p-value = {p:.4f}로, 일반적인 유의수준 0.05보다 작아 통계적으로 유의미한 상관관계가 있다고 볼 수 있습니다
- 즉, 여의도역 유동인구가 많을수록 아파트 시세가 높아지는 경향이 관찰되었습니다
